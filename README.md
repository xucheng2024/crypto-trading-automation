...
- **Universe:** the configured `OKX_INSTRUMENTS` list (`infrastructure/config/p5-enabled-instruments.json`), narrowed to pairs OKX lists as live USDT spot at startup and on each UTC+8 day. An empty list fails the baseline instead of widening to the whole market.
