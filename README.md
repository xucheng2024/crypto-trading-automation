...
The runtime does not create OKX algo-trigger orders. Buy signals are evaluated from live ticker observations, then admitted and submitted through the single order coordinator.

## Strategy: panic rebound

- **Universe:** every normally trading OKX USDT spot crypto pair (`instCategory` 1), discovered at startup and refreshed each UTC+8 day, excluding USD/fiat stablecoins, pairs listed less than seven days ago and pairs with an announced expiry (`expTime`). Blacklisted and announcement-delisted pairs are removed from the count as well as from buying (`OKX_INSTRUMENTS` no longer drives trading).
- **Count:** each UTC+8 day, a pair counts once its last trade reaches its day open × 0.82 (OKX `sodUtc8`). Pairs are ranked by the exchange time of that first touch; blacklisted and delisting pairs never count. After a restart, 5m candles only bound a missed touch to a five-minute window, so pairs whose order could affect the first two are skipped until two earlier touches are certain.
...
- **State:** day opens and first touches live in `panic_daily_instruments`; a restart backfills missed first touches from 5m candles before trading resumes.
...
# Current managed positions: start the VNet positions-read job (tens of seconds).
npm run ops:status -- positions --request

# One instrument's durable timeline: start the VNet timeline-read job.
...
# Latest production workflow, compact job/approval status, active revision and migration Runner.
npm run ops:status -- deploy

# VNet Runner Container App, GitHub registration, readiness, restarts and PAT presence.
npm run ops:status -- runner

# A specific failed deployment with a bounded diagnostic excerpt.
npm run ops:status -- deploy --run-id 31872057181 --details
...
Decision telemetry distinguishes normal market waiting (`ABOVE_COUNT_PRICE`, `ABOVE_BUY_PRICE`, `ASK_ABOVE_LIMIT`, `DAILY_OPEN_PENDING`), policy states (`SKIPPED_FIRST_TWO`, `PRIOR_POSITION_OPEN`, `CAPITAL_EXHAUSTED`), safety/data blockers, opportunities and execution events. An admitted BUY receives a deterministic `decisionId`, which is carried through coordinator guards, the durable attempt, `clOrdId`, API outcome and confirmed fill. Structured `block_evidence` traces expose the stage, reason code and available market, freshness, capacity, sizing and routing evidence without parsing error text. Block reports classify events as `LIKELY_RECOVERABLE`, `MARKET_MOVED`, or `SAFETY_BOUNDARY`, summarize stage coverage, and report the smallest exact capacity gap when sizing evidence is available. With `--details`, activity and report output link each instrument through candidate, blocker, persistence, API and fill stages; default output remains a compact aggregate. Results remain limited to retained App Insights telemetry; unavailable evidence must not be inferred. `positions` is the exception: `positions --request` starts the Manual Container Apps job `${engine%-engine}-positions-read` inside the existing VNet, runs a read-only SELECT on unclosed BUY fills, and returns a redacted per-instrument aggregate (remaining USD cost at durable fill prices, not real-time market value). It refuses an ambiguous multi-account scope. `trade --instrument X --request` starts `${engine%-engine}-timeline-read` the same way and returns that instrument's redacted durable timeline.
...
`deploy` and `runner` are also read-only. `deploy` summarizes the latest production workflow (or `--run-id`), failed steps, pending environment approvals, the active production revision and the VNet migration Runner. It distinguishes `IN_PROGRESS`, `SUCCEEDED`, and `FAILED`; an unavailable Azure component is reported without hiding the workflow result. Failed log excerpts are fetched only with `--details` and are capped at 12 diagnostic lines. Runner restart counts include normal ephemeral one-job process cycles. `snapshot` attributes `owner_lost SESSION_ADVISORY_LOCK_LOST` from an inactive OFF revision to `EXPECTED_OFF_TRANSITION`; current-revision and other inactive-revision severe traces remain visible and actionable.
