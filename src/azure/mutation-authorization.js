...
export function authorizeMutation({ tradingMode, mutation, ownerGuard, recoveryState, dependencies = {} }) {
  ...
  if (tradingMode === "OFF") return { allowed: false, reason: MUTATION_REASON.MODE_OFF };
  if (tradingMode === "EXIT_ONLY" && !EXIT_MUTATIONS.has(mutation)) {
    return { allowed: false, reason: MUTATION_REASON.MODE_EXIT_ONLY };
  }
