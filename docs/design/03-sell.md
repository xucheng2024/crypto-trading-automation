...
- 配置交易对上的 ACCOUNT SELL fill 按 `(inst_id,trade_id)` 先统一保存为 PENDING，不因“当前还没看到 managed BUY”立即判为多余。系统先完成覆盖该 sell `fillTime` 的连续 REST fills 重叠回补，并要求相关 SPOT/MARGIN fills watermarks 的较小值已越过该时间；每条 instType 流在成功且未满页的读取后，最多推进到读取开始时间减 5 分钟的保守完成边界，因此无成交的流也不会永久停滞，且不能用一个 instType 的成交时间代替另一个。这样更早发生但更晚到达的 ACCOUNT BUY 会先入账。若同 base 存在 PREPARED/SUBMITTED/UNKNOWN 系统退出 attempt，继续保持 PENDING。
