...
- 每个订单 attempt 和最终结果；
- partial/fill/reconciliation，包括成交价、OKX 返回的 fee/feeCcy 和成交后最大不利幅度；实际 fee 只用于审计，不进入规划公式；
- instrument protection 和 sync watermarks；不持久化借款利息或债务状态。
...
- BUY：PRICE_OUTSIDE、CANDLE_MISSING、BREAKOUT_NOT_CONFIRMED、ASK_ABOVE_LIMIT、YESTERDAY_GAIN_SKIPPED、STRATEGY_POSITION_EXISTS、BLACKLISTED、SNAPSHOT_STALE、MANAGED_EXPOSURE_STALE、MANAGED_QUOTE_REFRESH_FAILED、CLOCK_UNTRUSTED、ACCOUNT_RISK_UNSAFE、AUTO_LOAN_OFF、MAX_AVAIL_LOW、LEVERAGE_LIMIT、LEVERAGE_BREACH、RATE_LIMITED、IOC_ZERO/PARTIAL/FILLED、ORDER_UNKNOWN、HASH_COLLISION。
