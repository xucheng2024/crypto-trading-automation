...
const POLICY_REASONS = new Set(["SKIPPED_FIRST_TWO", "PRIOR_POSITION_OPEN", "CAPITAL_EXHAUSTED", "DAY_CLOSED", "ACTIVE_BUY_ATTEMPT", "DUPLICATE_MARKET_SNAPSHOT", "COUNT_HIT_RECORDED", "INSTRUMENT_PROTECTED"]);
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const positionsRead = options.command === "report"
    ? runPositionsCommand({ resourceGroup, app: appName, timeoutMs: 60_000, commandTimeoutMs: 5_000 }).then((result) => ({ result })).catch((error) => ({ unavailable: true, error: redactOperationalError(error.message) }))
    : null;
