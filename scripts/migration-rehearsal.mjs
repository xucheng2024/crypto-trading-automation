...
const contents = await Promise.all(postgresMigrations.map((name) => readFile(`migrations/postgres/${name}`, "utf8")));
...
try {
  ...
  const report = { schemaHash, migrationCount: postgresMigrations.length, contentHash: converted.content_hash, protectionRows: converted.instrument_protection.length, configArtifactHash: converted.content_hash, importedLegacyOrders: 0, idempotent: true, rollbackOnConflict: true };
