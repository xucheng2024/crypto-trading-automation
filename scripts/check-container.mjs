...
for (const required of ["FROM node:22.14.0-alpine", "npm ci --omit=dev", "USER trading", "TRADING_MODE=OFF", "HEALTHCHECK", "ENTRYPOINT"]) if (!dockerfile.includes(required)) throw new Error(`Dockerfile missing ${required}`);
