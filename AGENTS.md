...
- Use `trade --instrument X --request` (or `timeline`) for one instrument's durable execution timeline via the VNet `timeline-read` job.
...
- Treat `ABOVE_COUNT_PRICE`, `ABOVE_BUY_PRICE`, `ASK_ABOVE_LIMIT`, and `DAILY_OPEN_PENDING` as normal market waiting, and `SKIPPED_FIRST_TWO`, `PRIOR_POSITION_OPEN`, `CAPITAL_EXHAUSTED`, and `DAY_CLOSED` as policy skips; distinguish policy skips, safety/data blockers, opportunities, and execution events.
