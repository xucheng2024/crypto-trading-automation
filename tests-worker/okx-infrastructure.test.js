...
import { OkxRestClient, assertOkxResponse, classifyBatchResponse, classifyCrossFill, safeOkxFailure, safeOkxSummary, validateAccountProfile } from "../src/infrastructure/okx/rest-client.js";
import { OkxPrivateWsClient, OkxPublicWsClient, OkxWsReconnectBudget } from "../src/infrastructure/okx/ws-client.js";
...
test("P5 max-avail failures expose only structured, redacted diagnostics", async () => {
  ...
  await assert.rejects(client.maxAvailSize("BTC-USDT"), (error) => {
    assert.deepEqual(error.diagnostic, { failureClass: "HTTP_ERROR", endpoint: "/api/v5/account/max-avail-size", durationMs: 0, attempts: 1, httpStatus: 403, okxCode: "50100", okxMessageClass: "UNCLASSIFIED", okxSummary: "sensitive upstream text", responseClass: undefined });
    return true;
  });
  ...
  await assert.rejects(malformed.maxAvailSize("BTC-USDT"), (error) => {
    assert.deepEqual(error.diagnostic, { failureClass: "RESPONSE_INVALID", endpoint: "/api/v5/account/max-avail-size", durationMs: 0, attempts: 1, httpStatus: undefined, okxCode: undefined, okxMessageClass: undefined, okxSummary: undefined, responseClass: "EMPTY_DATA" });
    return true;
  });
  assert.deepEqual(safeOkxFailure(new DOMException("request timed out", "TimeoutError"), { endpoint: "/api/v5/account/max-avail-size", durationMs: 12, attempts: 1 }), { failureClass: "TIMEOUT", endpoint: "/api/v5/account/max-avail-size", durationMs: 12, attempts: 1, httpStatus: undefined, okxCode: undefined, okxMessageClass: undefined, okxSummary: undefined, responseClass: undefined });
  ...
  assert.equal(safeOkxSummary("Invalid token=abc123456789012345678901 IP 203.0.113.4 account 123456789 https://example.test/x"), "Invalid token=[REDACTED] IP [IP] account [ID] [URL]");
});
...
test("P5 REST transport records shared throttle delay and max-avail critical-path wait", async () => {
  ...
  const client = new OkxRestClient({ credentials, requestGapMs: 60, slo, clock: { nowMs: () => now }, sleep: async (ms) => { now += ms; }, fetcher: async (url) => ok([{ instId: new URL(url).searchParams.get("instId"), availBuy: "0", availSell: "0" }]) });
...
test("P1 Private WS keeps exponential backoff until 60 seconds stable", async () => {
  const sockets = []; const privateEvents = []; const timers = fakeTimers();
...
test("P1 Public WS follows the daily universe with incremental ticker subscriptions", () => {
  ...
  sockets[0].emit("message", JSON.stringify({ event: "unsubscribe", arg: { channel: "tickers", instId: "OLD-USDT" } }));
  ack({ channel: "tickers", instId: "NEW-USDT" });
  assert.equal(sockets.length, 1, "incremental ACKs never force a reconnect");
  assert.equal(client.snapshot().baseline, true);
  client.reconnect(); timers.runTimeouts(); sockets[1].emit("open");
  assert.deepEqual(JSON.parse(sockets[1].sent[0]).args.filter((arg) => arg.channel === "tickers").map((arg) => arg.instId), ["BTC-USDT", "NEW-USDT"], "a reconnect subscribes the current universe");
});
